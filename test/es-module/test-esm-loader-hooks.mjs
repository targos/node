...
describe('Loader hooks', { concurrency: true }, () => {
  it('are called with all expected arguments', async () => {
    const { code, signal, stdout, stderr } = await spawnPromisified(execPath, [
      '--no-warnings',
      '--loader',
      fixtures.fileURL('/es-module-loaders/loader-hooks-input.mjs'),
      '--loader',
      fixtures.fileURL('/es-module-loaders/loader-store-data.mjs'),
      fixtures.path('/es-modules/hooks-input.mjs'),
    ]);
    ...
    assert.match(lines[1], /{"url":"file:\/\/\/.*\/hooks-input\.mjs","format":"test","shortCircuit":true}/);
    assert.match(lines[2], /{"source":{"type":"Buffer","data":\[.*\]},"format":"module","shortCircuit":true}/);
    assert.match(lines[3], /{"url":"file:\/\/\/.*\/experimental\.json","format":"test","shortCircuit":true}/);
    assert.match(lines[4], /{"source":{"type":"Buffer","data":\[.*\]},"format":"json","shortCircuit":true}/);
  });
