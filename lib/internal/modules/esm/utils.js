...
async function initializeHooks() {
  ...
  debug('initializing internal ESMLoader');
  ...
  for (let i = 0; i < customLoaderPaths.length; i++) {
    ...
    debug('importing custom loader', customLoaderPath);
  ...
  debug('custom loaders importation complete');

  const { Hooks } = require('internal/modules/esm/hooks');
  const hooks = new Hooks(importedCustomLoaders);

  debug('hooks inititalized; triggering global preloads');

  hooks.preload();

  debug('global preloads completed');

  // Preload after loaders are added so they can be used
  const preloadModules = getOptionValue('--import');

  if (preloadModules) {
    debug('triggering preload modules (which are somehow different to preloads).');
