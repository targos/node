...
async function customizedModuleWorker(lock, syncCommPort, errorHandler) {
  ...
  // DEBUG: let's confirm that using a write operation would let the test pass instead of fail.
  ...
  AtomicsNotify(lock, WORKER_TO_MAIN_THREAD_NOTIFICATION); // Unlock the main thread now that we are ready.
  ...
  async function handleMessage({ method, args, port }) {
    ...
    if ((port == null ? pendingAsyncResponses : pendingAsyncResponses++) === 0) {
      // We are about to yield the execution with `await ReflectApply` below.
      // In case the code following the `await` never runs, we remove the message
      // handler so the `beforeExit` event can be triggered.
    ...
    if ((port == null ? pendingAsyncResponses : --pendingAsyncResponses) === 0) {
      // All the messages have been handled before the event loop died, we can go back to the initial setup.
      syncCommPort.on('message', handleMessage);
      clearImmediate(immediate);
    }
