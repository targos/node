...
// [1] AtomicsNotify can happen faster than the other client is prepared to receive. Notify returns
// the number of clients notified, so if it initially fails, keep trying (it will eventually
// succeed).
...
async function customizedModuleWorker(lock, syncCommPort, errorHandler) {
  ...
  while (AtomicsNotify(lock, WORKER_TO_MAIN_THREAD_NOTIFICATION) === 0); // [1]
