...
WORKER_TO_MAIN_THREAD_NOTIFICATION is only used to send notifications, its value
is going to increase every time the worker send a "time-sensitive" notification
to the main thread. Time-sensitive here means a notification that is sent to
inform a message was sent.
...
NUMBER_OF_MESSAGES_IN_TRANSIT is used to communicate the number of messages
sent by the main thread and not yet acknowledged by the worker thread. It is
also used to wake up the worker thread when it locks itself.

NUMBER_OF_INCOMING_ASYNC_RESPONSES is used to track how many async responses
the main thread should expect.
...
module.exports = {
  ...
  NUMBER_OF_MESSAGES_IN_TRANSIT: 2,
