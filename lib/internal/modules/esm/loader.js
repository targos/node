...
/**
 * Multiple instances of ESMLoader exist for various, specific reasons (see code comments at site).
 * In order to maintain consitency, we use a single worker (sandbox), which must sit apart of an
 * individual ESMLoader instance.
 */
let hooksProxy;
...
class ESMLoader {
  ...
  registerCustomLoaders(customLoaders) {
    if (hooksProxy) { return; }

    if (!customLoaders.length) { return; } // Nothing to do.

    const { HooksProxy } = require('internal/modules/esm/hooks');
    hooksProxy = new HooksProxy(); // The worker adds custom hooks as part of its startup.
  }
  ...
  getModuleJob(specifier, parentURL, importAssertions) {
    ...
    // The Node.js default resolve hook does not use import assertions, so we need to clone only if
    // there are user-provided loaders.
    if (hooksProxy) { // This exists only when there are custom hooks.
    ...
    const { format, url } = this.resolve(specifier, parentURL, importAssertionsForResolve);
  ...
  resolve(
    ...
    if (hooksProxy) {
      return hooksProxy.makeRequest('resolve', originalSpecifier, parentURL, importAssertions);
    }
  ...
  async load(url, context) {
    let loadResult;
    if (hooksProxy) {
      loadResult = await hooksProxy.makeRequest('load', url, context);
    } else {
      if (!this.#defaultLoad) {
