...
class DefaultModuleLoader {
  ...
  getModuleJob(specifier, parentURL, importAssertions) {
    const { format, url } = this.resolve(specifier, parentURL, importAssertions);
  ...
  async load(url, context) {
    ...
    const result = await this.#defaultLoad(url, context);
  ...
  importMetaInitialize(meta, context) {
    ...
    meta = this.#importMetaInitializer(meta, context, this.resolve);
    return meta;
  }
}
...
class CustomizedModuleLoader extends DefaultModuleLoader {
   ...
   */
  async load(url, context) {
    const result = await hooksProxy.makeRequest('load', url, context);
    this.validateLoadResult(url, result?.format);
    return result;
  }
}
...
class ModuleLoader {
   ...
   * @returns {instanceof BaseModuleLoader | instanceof CustomizedModuleLoader}
   */
  constructor(useCustomLoadersIfPresent = true) {
    if (useCustomLoadersIfPresent) {
      const userLoaderPaths = getOptionValue('--experimental-loader');
      if (userLoaderPaths.length > 0) {
        emitExperimentalWarning('Custom ESM Loaders');
        return new CustomizedModuleLoader(userLoaderPaths);
      }
    }
