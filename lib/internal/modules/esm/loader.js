...
class HooksProxy {
  ...
  makeRequest(type, ...args) {
    ...
    debug(`sending ${type} request to worker (and then sleeping) with args`, ...args);
...
class ESMLoader {
  ...
  addCustomLoaders() {
    hookProxy ??= new HooksProxy(); // The worker adds custom hooks as part of its startup.
  }

  preload() {
    hookProxy?.makeRequest('preload');
  }
  ...
  async getModuleJob(specifier, parentURL, importAssertions) {
    ...
    if (hookProxy) { // This exists only when there are custom hooks.
  ...
  async resolve(
    ...
    if (hookProxy) {
      return hookProxy.makeRequest('resolve', originalSpecifier, parentURL, importAssertions);
    }
  ...
  async load(url, context) {
    let loadResult;
    if (hookProxy) {
      loadResult = await hookProxy.makeRequest('load', url, context);
    } else {
      if (!this.#defaultLoad) {
  ...
  importMetaInitialize(meta, context) {
    if (hookProxy) {
      return hookProxy.makeRequest('importMetaInitialize', meta, context);
    }
