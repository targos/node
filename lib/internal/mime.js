...
/**
 * Converts a string to lower case, only considering A-Z characters.
 * @param {string} str
 * @returns {string}
 */
...
/**
 * @param {string} str
 * @returns {[type: string, subtype: string, position: number]}
 */
...
const encode = (value) => {
  ...
  const needsEncode = SafeStringPrototypeSearch(value, NOT_HTTP_TOKEN_CODE_POINT) !== -1;
  ...
  #parse() {
    ...
    while (position < endOfSource) {
      ...
      if (char === '"') {
        ...
        parameterValue = unescapeBackslashes(inside);
